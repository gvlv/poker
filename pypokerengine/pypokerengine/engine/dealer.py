...
class Dealer:
  ...
  def __init__(self, small_blind_amount=None, initial_stack=None, ante=None, reset_stack=False):
  ...
  def register_player(self, player_name, algorithm, initial_stack=None):
    if initial_stack is None:
        initial_stack = self.initial_stack
    self.__config_check()
    uuid = self.__escort_player_to_table(player_name, initial_stack)
  ...
  def start_game(self, max_round):
    ...
    for round_count in range(1, max_round+1):
      if self.reset_stack:
          for s in table.seats.players:
              if "Dummy" not in s.name:
                s.stack = self.initial_stack
  ...
  def __escort_player_to_table(self, player_name, initial_stack):
    uuid = self.__fetch_uuid()
    player = Player(uuid, initial_stack, player_name)
